...
/**
 * Services a standard visualization request, which involves templates. This
 * will return a simple error message and a 501 if there is no jena Model.
 *
 * @author cdtank
 */
...
//						+ " -- " + vitroRequest.getRequestURI().substring(vitroRequest.getContextPath().length()));
     ...
     * vivo.com/vis/author-network/shortURI vivo.com/vis/grant-graph/shortURI
     * etc. So first we obtain the request url which can be used to extract the
     * requested visualization and the subject of the visualization. So the
     * below pattern matcher will take "/vis/<vis-name>/<shortURI>" as an input.
     */
