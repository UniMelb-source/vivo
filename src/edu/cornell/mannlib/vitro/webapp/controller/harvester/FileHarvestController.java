...
public class FileHarvestController extends FreemarkerHttpServlet {
    ...
    /**
     * Stores information about the Harvester thread for a particular user
     * session.
     */
    ...
    /**
     * A list of known job parameters (that is, "job=" values from the query
     * string which we will accept from the browser). This should be filled in
     * the static initializer and then never written to again.
     */
    private static final List<String> knownJobs = new ArrayList<String>();
    /**
     * Relative path from the VIVO Uploads directory to the root location where
     * user-uploaded files will be stored. Include final slash.
     */
    private static final String PATH_TO_UPLOADS = "harvester/";
    /**
     * Relative path from the Harvester root directory to the main area reserved
     * for the VIVO File Harvest feature. Include final slash.
     */
    private static final String PATH_TO_FILE_HARVEST_ROOT = "vivo/";
    /**
     * Relative path from the Harvester root directory to the directory where
     * user-downloadable template files are stored. Include final slash.
     */
    public static final String PATH_TO_TEMPLATE_FILES = PATH_TO_FILE_HARVEST_ROOT + "templates/";
    /**
     * Relative path from the Harvester root directory to the directory
     * containing the script templates. Include final slash.
     */
    ...
    /**
     * Fill the known job types list. Any time a new job type is added, we need
     * to make sure this method is adding it to the list. By "new job type" is
     * meant a new "job=" parameter that we understand when we see it in the
     * query string. This typically means we have also handled seeing this
     * parameter in the getJob() method of this class.
     *
     * The exception to all this is a new CSV job, which is entirely handled by
     * adding a new CsvFileHarvestJob.JobType enum value. This method as well as
     * this class's getJob() method already handle the rest.
     */
    ...
    /**
     * Returns the path on this machine of the area within Harvester reserved
     * for File Harvest.
     *
     * @return the path on this machine of the area within Harvester reserved
     * for File Harvest
     */
    ...
    /**
     * Gets the FileHarvestJob implementation that is needed to handle the
     * specified request. This will depend on the type of harvest being
     * performed (CSV, RefWorks, etc.)
     *
     * @param vreq the request from the browser
     * @param jobParameter the POST or GET parameter "job". Might not be
     * available in vreq at this point, thus we are requiring that it be sent
     * in.
     * @return the FileHarvestJob that will provide harvest-type-specific
    ...
    /**
     * Gets the location where we want to save uploaded files. This location is
     * in the VIVO uploads directory under "harvester", and then in a directory
     * named by the user's session ID as retrieved from the request. The path
     * returned by this method will end in a slash (/).
     *
     * @param vreq the request from which to get the session ID
     * @return the path to the location where uploaded files will be saved. This
     * path will end in a slash (/)
     */
    ...
    /**
     * This is for when the user clicks the "Upload" button on the form, sending
     * a file to the server. An HTTP post is redirected here when it is
     * determined that the request was multipart (as this will identify the post
     * as a file upload click).
     *
    ...
    /**
     * This is for when the user clicks the "Harvest" button on the form,
     * sending a file to the server. An HTTP post is redirected here when an
     * isHarvestClick parameter is contained in the post data and set to "true".
     *
     * @param request the HTTP request
     * @param response the HTTP response
     */
    ...
    /**
     * This is for posts automatically sent by the client during the harvest, to
     * check on the status of the harvest and return updated log data and
     * whether the harvest is complete or still running. An HTTP post is
     * redirected here when an isHarvestClick parameter is contained in the post
     * data and set to "false".
     *
     * @param request the HTTP request
     * @param response the HTTP response
     */
    ...
    /**
     * Returns the location in which the ready-to-run scripts, after having
     * template replacements made on them, will be placed. Final slash included.
     *
    ...
    /**
     * Handles a name conflict in a directory by providing a new name that does
     * not conflict with the name of a file already uploaded.
     *
     * @param filename the name of the file to be added to the directory
     * @param directory the directory where the file should be added, in which
     * to check for files of the same name
     * @return a filename that does not conflict with any files in the
     * directory. If the filename parameter works, then that is returned.
     * Otherwise a number is appended in parentheses to the part of the file
     * name prior to the final "." symbol (if one exists).
     */
    ...
    /**
     * Parse an additions file (RDF/XML) to get the URIs of newly-harvested
     * data, which will be sent to the browser and displayed to the user as
     * links.
     *
    ...
    /**
     * Parse an XML node for all subnodes with qualified name "rdf:type", and
     * return each's "rdf:resource" value in a list.
     *
    ...
    /**
     * If a session info object exists for this session ID, abort the thread if
     * it is still running and remove the object.
     *
    ...
    /**
     * If all goes according to plan, clearSessionInfo() should be called once
     * the client gets the last bit of information from the harvest. However, if
     * the client doesn't request it (because the browser was closed, etc.) then
     * the method will never get called. This method gets called every time the
     * page is initially loaded, to look for session data that is 6 hours old or
     * more, and remove it.
     */
     ...
     *
     * @param fatalError whether the fatal error flag should be set on this
     * object
    ...
    /**
     * Information relating to a particular user session, created just before
     * the harvester thread is starting.
     *
     * @author mbarbieri
     */
    private class SessionInfo {
        ...
        /**
         * Harvester output that has not yet been sent back to the browser, for
         * this user session.
         */
    ...
    /**
     * Provides a way of throwing an exception whose message it is OK to display
     * unedited to the user.
     */
