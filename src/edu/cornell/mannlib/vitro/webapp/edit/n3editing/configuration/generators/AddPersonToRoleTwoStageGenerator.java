 ...
 *
 *
 * Important: This form cannot be directly used as a custom form. It has
 * parameters that must be set. See addClinicalRoleToPerson.jsp for an example.
 *
 ...
 * documented, required methods to override
 *
 * AddRoleToPersonTwoStageGenerator is abstract, each subclass will need to
 * configure: From the old JSP version:
 *
 * showRoleLabelField boolean roleType URI roleToActivityPredicate URI
 * activityToRolePredicate URI roleActivityType_optionsType
 * roleActivityType_objectClassURI roleActivityType_literalOptions
 *
 * For the new generator version: template
 *
 */
public abstract class AddPersonToRoleTwoStageGenerator extends BaseEditConfigurationGenerator implements EditConfigurationGenerator {

    private Log log = LogFactory.getLog(AddPersonToRoleTwoStageGenerator.class);
    ...
    /**
     * In the case of literal options, subclass generator will set the options
     * to be returned
     */
    abstract HashMap<String, String> getRoleActivityTypeLiteralOptions();

    /**
     * Each subclass generator will return its own type of option here: whether
     * literal hardcoded, based on class group, or subclasses of a specific
     * class The latter two will apparently lend some kind of uri to
     * objectClassUri ?
     */
    ...
    /**
     * If true an input should be shown on the form for a label for the role
     * context node TODO: move this to the FTL and have label optional.
     */
...
//    abstract String getActivityToRolePredicate();            
    ...
    public EditConfigurationVTwo getEditConfiguration(VitroRequest vreq, HttpSession session) {
        ...
        editConfiguration.setN3Optional(list(
                "?role <http://vivoweb.org/ontology/core#researcherRoleOf>" + /*getRoleToActivityPlaceholder() + */ " ?roleActivity .\n"
                + "?roleActivity <http://vivoweb.org/ontology/core#hasResearcherRole>" + /*getActivityToRolePlaceholder() + */ " ?role .",
